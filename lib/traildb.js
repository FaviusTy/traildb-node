 ...
 */
function TrailsIterator(tdb, options = {}) {
  this.tdb = tdb;
...
TrailsIterator.prototype[Symbol.iterator] = function () {
  const {tdb, options} = this;
  const numTrails = lib.tdb_num_trails(tdb._db);
  let i = 0;

  return {
    next: () => {
      if (i >= numTrails) return { done: true };
      return {
        value: tdb.trail(i, options),
        done: i++ > numTrails
      };
    }
  };
 ...
 */
function EventsIterator(trail, options = {}) {
  this.trail = trail;
  this.options = options;
  const db = this.trail.tdb._db;

  this.cursor = lib.tdb_cursor_new(db);
  /*\
  |*| Filter logic
  \*/
  if (typeof this.options.filter != 'undefined') {
    this.filter = new TrailDBFilter(Object.assign({db: db}, this.options));
    const err = lib.tdb_cursor_set_event_filter(this.cursor, this.filter);
    if (err) throw new TrailDBError(`Unable to set event filter: ${this.trail.id}`);
  }
  const r = lib.tdb_get_trail(this.cursor, this.trail.id);
  if (r) throw new TrailDBError(`Failed to open trail cursor: ${this.trail.id}`);
  return this;
}
...
EventsIterator.prototype[Symbol.iterator] = function () {
  const {cursor, filter, options: {toMap}, trail: {tdb}} = this;

  return {
    next: () => {
      const eventPtr = lib.tdb_cursor_next(cursor);

      if (!eventPtr || !eventPtr.length) {
        if (typeof this.options.filter != 'undefined') lib.tdb_event_filter_free(filter);
        lib.tdb_cursor_free(cursor);
        return { done: true };
      }
      ...
      for (let i = 0, addr, value; i < event.num_items; i++) {
        ...
        value = tdb.getItemValue(addr);
        list.push(value);

        if (toMap) map[tdb.getItemKey(addr)] = value;
      }
 ...
 */
function TrailDBConstructor(options) {
  ...
  options.fieldNames.forEach((field, i) => {
...
TrailDB.prototype.trail = function (i, options) {
  ...
  if (r) throw new TrailDBError(`Failed to open trail: ${i}`);
 ...
 */
function TrailDBFilter(options) {
  /*\
  ...
  \*/
  if (options.filter.length <= 0) return filter;
  /*\
  ...
  for (var i = 0; i < options.filter.length; i++) {
    ...
    if (validFilterClauses) {
      ...
      \*/
      if (newClause && lib.tdb_event_filter_new_clause(filter)) {
    ...
    \*/
    var neg = 0;
    if (options.filter[i].neg === true) neg = 1;
