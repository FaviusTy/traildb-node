...
class EventCursor {
  ...
  resetTrail(trailID) {
    if(trailID !== 0 && !trailID) throw new TrailDBError(`undefined trailID: ${trailID}`);
    ...
    return {
      next: () => {
        ...
        for (let i = 0; i < event.num_items; i++) {
          ...
          this.toMap ? value[this.tdb.getItemKey(addr)] = this.tdb.getItemValue(addr) : value.push(this.tdb.getItemValue(addr));
        }

        return { value: {timestamp: event.timestamp, values: value}, done: false };
      }
    }
  }
